  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {cards.map((card, i) => (
        <div
          key={card.label}
          className={`bg-gradient-to-br ${card.color} rounded-xl border ${card.borderColor} p-6 animate-fade-in`}
          style={{ animationDelay: `${i * 100}ms` }}
        >
          <div className={`${card.iconColor} mb-3`}>{card.icon}</div>
          <p className="text-sm text-[#888]">{card.label}</p>
          <p className="text-3xl font-bold mt-1 text-white animate-count">{card.value}</p>
        </div>
      ))}
    </div>
  );
