...
export default function StatsCards({
  ...
  const cards = [
    ...
    },
    {
      ...
      color: "from-[#ff4d00]/20 to-[#ff4d00]/5",
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {cards.map((card, i) => (
        <div
          key={card.label}
          className={`bg-gradient-to-br ${card.color} rounded-xl border ${card.borderColor} p-4 animate-fade-in`}
          style={{ animationDelay: `${i * 100}ms` }}
        >
          <div className={`${card.iconColor} mb-2`}>{card.icon}</div>
          <p className="text-xs text-[#888]">{card.label}</p>
          <p className="text-2xl font-bold text-white animate-count">{card.value}</p>
        </div>
      ))}
    </div>
  );
